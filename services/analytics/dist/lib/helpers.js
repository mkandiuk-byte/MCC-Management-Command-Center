...
export function detectNetwork(id) {
    if (/^\d{15,25}$/.test(id))
        return 'Meta';
    if (/^\d{9,14}$/.test(id))
        return 'Google';
    if (/^[A-Za-z0-9_-]{18,25}$/.test(id))
        return 'TikTok';
    return 'Other';
}
...
export function parseOfferName(name) {
...
export function normalizeSchema(raw) {
    ...
    if (s === 'prelandings')
        return 'prelanding';
    if (s === 'prelanding_landings')
        return 'prelanding+landing';
    return s || 'direct';
}
// ─── Date formatting ──────────────────────────────────────────────────────────
...
// ─── Derived metrics ─────────────────────────────────────────────────────────
export function deriveMetrics(clicks, conversions, revenue, cost) {
...
export function classifyCampaign(type, cost, clicks, conversions, roi) {
    if (type !== 'offer')
        return 'no_data';
    if (cost < MIN_COST_FOR_DATA || clicks < MIN_CLICKS_FOR_DATA)
        return 'no_data';
    if (conversions === 0)
        return 'failed';
    if (roi >= 0)
...
export function parseCostModel(raw) {
    const v = raw.toLowerCase().trim();
    if (['cpc', 'cpa', 'cpm', 'revshare', 'fixed'].includes(v))
        return v;
    return 'unknown';
}
...
export function buildRecommendation(status, type, cost, clicks, conversions, cpa, roi, cr, cpc, costModel, paybackMonths) {
    if (type === 'app') {
        const parts = [
        ...
        if (clicks > 0) {
            ...
            if (conversions > 0)
                parts.push(`Прохідність (FLOW): ${conversions.toLocaleString()} подій, CR ${cr.toFixed(2)}%.`);
            else
                parts.push('Конверсії не відстежуються — це нормально для routing-кампаній.');
        }
        return parts.join(' ');
    }
    if (type !== 'offer') {
    ...
    const parts = [];
    if (costModel === 'cpc')
        parts.push(`Модель: CPC ($${cpc > 0 ? cpc.toFixed(3) : '?'}/клік).`);
    else if (costModel === 'revshare')
        parts.push('Модель: RevShare — трафік-сорс бере % від доходу.');
    else if (costModel === 'cpm')
        parts.push(`Модель: CPM ($${cpc > 0 ? (cpc * 1000).toFixed(2) : '?'}/1000 показів).`);
    if (conversions === 0) {
        parts.push(`За вказаний період нема конверсій при витратах $${cost.toFixed(0)}. Перевір постбэк і налаштування оффера.`);
    }
    else {
        if (roi >= 0)
            parts.push(`CPA $${cpa.toFixed(0)}, ROI ${roi.toFixed(0)}% — кампанія прибуткова. Якщо CPA нижче еталону по гео — масштабуй бюджет.`);
        else
            parts.push(`CPA $${cpa.toFixed(0)}, ROI ${roi.toFixed(0)}% — кампанія збиткова. Оптимізуй ставки та тестуй нові креативи.`);
    }
