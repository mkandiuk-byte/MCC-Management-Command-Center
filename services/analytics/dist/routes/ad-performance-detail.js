...
function classifySignal(ftds, roi, roiTotal) {
    if (ftds < 5)
        return 'NEW';
    if (roi !== null && roi >= 0)
        return 'OK';
    if (roiTotal !== null && roiTotal < -30)
        return 'STOP';
    return 'WATCH';
}
export async function adPerformanceDetailRoutes(app) {
    app.get('/', async (req, reply) => {
        ...
        `;
            ...
            const name = t.group_name ? String(t.group_name) : null;
            // ── Daily ───────────────────────────────────────────────────────────────
            let dailyRows;
            if (groupBy === 'offer_id') {
                const offerId = parseInt(id, 10);
