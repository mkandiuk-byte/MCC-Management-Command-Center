...
function isCollectorCampaign(name) {
...
export async function chainRoutes(app) {
    app.get('/', async (req, reply) => {
      ...
      `;
            ...
            const campNodes = new Map();
            const cloakNodes = new Map();
            const offerNodes = new Map();
            // Edge weight maps (sourceId→targetId → clicks)
            const adToCamp = new Map();
            const adToCloak = new Map();
            const campToOffer = new Map();
            const cloakToOffer = new Map();
            // Drilldown link maps
            const adOfferRaw = new Map();
            const adCampRaw = new Map();
            for (const row of rows) {
                ...
                if (isCollectorCampaign(campName)) {
                    ...
                    if (cloakId) {
                        ...
                        if (!adCampRaw.has(adNid))
                            adCampRaw.set(adNid, new Set());
                ...
                if (offerNid) {
                    ...
                    on.revenue += revenue;
                    on.cost += cost;
                    if (!adOfferRaw.has(adNid))
                        adOfferRaw.set(adNid, new Set());
                    adOfferRaw.get(adNid).add(offerNid);
                }
            }
            // ── Finalise cr ───────────────────────────────────────────────────────────
            for (const n of campNodes.values())
                if (n.clicks > 0)
                    n.cr = (n.conversions / n.clicks) * 100;
            for (const n of cloakNodes.values())
                if (n.clicks > 0)
                    n.cr = (n.conversions / n.clicks) * 100;
            for (const n of offerNodes.values())
                if (n.clicks > 0)
                    n.cr = (n.conversions / n.clicks) * 100;
            // ── Layout: 3 columns ─────────────────────────────────────────────────────
            const H = 130;
            ...
            let ei = 0;
            const w = (c) => Math.max(1, Math.round((c / maxClicks) * 5));
            ...
            // ── Drilldown link maps ────────────────────────────────────────────────────
            const adOfferLinks = {};
            for (const [adNid, s] of adOfferRaw)
                adOfferLinks[adNid] = [...s];
            const adCampLinks = {};
            for (const [adNid, s] of adCampRaw)
                adCampLinks[adNid] = [...s];
            const data = {
                nodes,
