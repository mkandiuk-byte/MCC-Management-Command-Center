...
function classifyOffer(cost, clicks, conversions, roi) {
    if (cost < MIN_COST || clicks < MIN_CLICKS)
        return 'no_data';
    if (conversions === 0)
        return 'failed';
    if (roi >= 0)
...
function buildOfferRecommendation(status, cost, clicks, conversions, cpa, roi, cr) {
    const parts = [];
    if (conversions === 0) {
        ...
        if (roi >= 0)
            parts.push(`CPA $${cpa.toFixed(0)}, ROI ${roi.toFixed(0)}% — оффер прибутковий. Якщо CPA нижче еталону по гео — масштабуй.`);
        else
            parts.push(`CPA $${cpa.toFixed(0)}, ROI ${roi.toFixed(0)}% — оффер збитковий. Оптимізуй ставки та тестуй нові воронки.`);
    }
    ...
    app.get('/', async (req, reply) => {
        ...
        `,
            ...
            for (const r of offerRows) {
                ...
                if (!ltGroups.has(groupKey))
                    ltGroups.set(groupKey, new Map());
                const gm = ltGroups.get(groupKey);
                if (!gm.has(meta.landingType))
                    gm.set(meta.landingType, { cr: [], roi: [], cpa: [] });
