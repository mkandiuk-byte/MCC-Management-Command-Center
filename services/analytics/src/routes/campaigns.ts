...
import {
  detectCampaignType, classifyCampaign, buildRecommendation, parseCostModel,
  deriveMetrics, formatDate, type CampaignType, type CostModel,
} from '../lib/helpers.js'
...
export async function campaignsRoutes(app: FastifyInstance) {
  ...
  app.get('/', async (req, reply) => {
    ...
    try {
      ...
      const offerMap = new Map<number, Array<{
        name: string; clicks: number; conversions: number
        revenue: number; cost: number; profit: number; roi: number; cr: number; cpa: number; shareClicks: number
      }>>()
      for (const row of offerRows) {
        const cid = Number(row.campaign_id)
        if (!offerMap.has(cid)) offerMap.set(cid, [])
      ...
      for (const list of offerMap.values()) {
        ...
        if (total > 0) list.forEach(o => { o.shareClicks = (o.clicks / total) * 100 })
      }
      ...
      const campaigns = statsRows
        ...
        .map(r => {
          ...
          const campaignType: CampaignType = detectCampaignType(name)
          const costModel: CostModel = parseCostModel(String(r.cost_type ?? ''))
          ...
          const recommendation = buildRecommendation(
            status, campaignType, r.cost, r.clicks, r.conversions,
            m.cpa, m.roi, m.cr, m.cpc, costModel, paybackMonths,
          )
          return {
