...
const _cache = new Map<string, { data: GraphResponse; ts: number }>()

export async function graphCampaignRoutes(app: FastifyInstance) {
  ...
  app.get('/:id', async (req, reply) => {
    ...
    try {
      ...
      // ── Accumulate offer-level stats per stream ───────────────────────────
  ...
  app.post('/:id', async (req) => {
    const cid = parseInt((req.params as { id: string }).id, 10)
