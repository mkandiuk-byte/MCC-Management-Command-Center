...
// ─── Dimension → SQL expression + optional JOIN ───────────────────────────────

interface DimDef { expr: string; alias: string; join?: string }

const DIM_MAP: Record<string, DimDef> = {
...
const MEASURE_SQL: Record<string, string | null> = {
  clicks:          "COUNT(*)::int",
...
export async function reportRoutes(app: FastifyInstance) {
  ...
  app.get('/', async (req, reply) => {
    ...
    if (dims.length === 0) return reply.status(400).send({ error: 'Invalid or unsupported dimensions. Valid: ' + [...VALID_DIMS].join(', ') })
    if (measures.length === 0) return reply.status(400).send({ error: 'Invalid or unsupported measures. Valid: ' + [...VALID_MEASURES].join(', ') })
    ...
    try {
      ...
      const extraMeasures: string[] = []
      if (needClicks)      extraMeasures.push('COUNT(*)::int AS _clicks')
      if (needConversions) extraMeasures.push('(SUM(a.is_sale::int) + SUM(a.is_lead::int))::int AS _conversions')
      if (needRevenue)     extraMeasures.push('COALESCE(SUM(a.revenue), 0)::float AS _revenue')
      if (needCost)        extraMeasures.push('COALESCE(SUM(a.cost), 0)::float AS _cost')
      ...
      const rows = rawRows.map(row => {
        ...
        if (measures.includes('roi'))   r['roi']   = cost > 0 ? (profit / cost) * 100 : 0
        if (measures.includes('cr'))    r['cr']    = clicks > 0 ? (conversions / clicks) * 100 : 0
        if (measures.includes('cpa'))   r['cpa']   = conversions > 0 ? cost / conversions : 0
        if (measures.includes('epc'))   r['epc']   = clicks > 0 ? revenue / clicks : 0

        // Remove helper columns
        delete r['_clicks']; delete r['_conversions']; delete r['_revenue']; delete r['_cost']
        return r
      })
