...
export interface SparkPoint { day: string; spend: number; profit: number }

export async function adPerformanceSparklinesRoutes(app: FastifyInstance) {
  app.get('/', async (req, reply) => {
    ...
    const cached = await cacheGet<Record<string, SparkPoint[]>>(cacheKey)
      ...
      if (groupBy === 'offer_id') {
        rows = await sql`
          ...
          SELECT
            ...
            (a.datetime AT TIME ZONE 'UTC')::date::text                              AS day,
